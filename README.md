...
- 已实现：
  ...
  - 本地 mock 消息收发已打通（输入后追加到本地 `messages` 状态）。
  - 新消息交互已优化：不再无条件自动滚底；用户不在底部时显示“有新消息”按钮，点击后滚到底部。
  - 学习 memo 已补充：`useEffect` 触发时机与消息列表性能 TODO。

- 未实现：
  - 后端服务未实现（无 Express 服务、无 Socket.io 服务端、无 REST API）。
  - 前端未接入真实后端（无历史消息拉取、无 WebSocket 实时订阅/广播）。
  - 消息持久化未实现（数据库与消息存储链路未落地）。
  - 消息可靠性机制未实现（ack、重试、失败状态流转等）。
  - 大列表性能优化未实现（虚拟列表与历史分页仅记录为 TODO，尚未编码）。

- 后端结论：
  - 当前阶段仍是“前端 mock 驱动”的 UI/交互验证阶段。
  - `socket.io-client` 依赖已安装，但尚未在代码中完成实际连接与事件处理。
